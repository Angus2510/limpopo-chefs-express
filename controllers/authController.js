...
async function findUser(identifier) {
  const staffUser = await Staff.findOne({ $or: [{ email: identifier }, { username: identifier }] })
  ...
  if (guardianUser) {
    ...
    guardianUser.inactiveReason = hasActiveStudent ? null : 'No active students';
...
async function findUserById(id) {
  const staffUser = await Staff.findById(id).populate('roles', 'roleName').exec();
  if (staffUser) {
    return { user: staffUser, userType: 'staff' };
  }
...
const login = asyncHandler(async (req, res) => {
 ...
 let  needsAgreement = false;
  ...
  if (!user.agreementAccepted || !user.agreementAcceptedDate || new Date(user.agreementAcceptedDate) < oneMonthAgo) {
    needsAgreement = true;
  }
  ...
  if (user.profile && user.profile.avatar) {
    ...
    avatarUrl = `https://swartstudio.co.za/api/files/getFile?key=${avatarKey}`;
  }
  ...
  if (user.active) {
    ...
    if (userType === 'student') {
      tokenPayload.UserInfo.intakeGroup = user.intakeGroup.length > 0 ? user.intakeGroup[0]._id : null;
    }
  ...
  jwt.verify(
    ...
    async (err, decoded) => {
      if (err) {
        ...
        return res.status(403).json({ message: 'Forbidden - Refresh token invalid' });
      }
      ...
      if (user.refreshToken !== refreshToken) {
        ...
        return res.status(403).json({ message: 'Forbidden - Refresh token mismatch' });
      }
      ...
      if (user.active) {
        ...
        if (user.profile && user.profile.avatar) {
          ...
          avatarUrl = `https://swartdesign.co.za/api/files/getFile?key=${avatarKey}`;
        }
