...
exports.getStudenSorByIds = async (req, res) => {
  try {
    ...
    // Fetch students
    const students = await Student.find({ _id: { $in: idsArray } })
      .select('admissionNumber profile.firstName profile.lastName profile.dateOfBirth campus qualification')
    ...
    const results = await Result.find({ 'results.student': { $in: idsArray } })
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=students_results.zip');
    ...
    const generatePdfForStudent = async (student) => {
      try {
        let studentResults = results.filter(result =>
          result.results.some(r => r.student._id.toString() === student._id.toString())
        );
          ...
          <html>
            ...
            <body>
              <div class="header">
                ...
                <div class="qualification">${student.qualification.map(q => q.title).join(', ')}</div>
              </div>
              ...
              <div class="results">
                <table class="results-table">
                  ...
                  ${studentResults.map(result => {
                    ...
                    const outcomeTitle = result.outcome ? result.outcome.title : 'Unknown';

                    if (result.resultType === 'Theory' && studentResult.testScore === 0 && studentResult.taskScore === 0) {
                      return '';
                    }

                    if (result.resultType !== 'Theory' && studentResult.score === 0) {
                      return '';
                    }
                    ...
                    if (result.resultType === 'Theory') {
                      const averageScore = (studentResult.testScore + studentResult.taskScore) / 2;
        ...
        const fileName = `${student.admissionNumber}_${new Date().toISOString().split('T')[0]}_sor.pdf`;
        archive.append(pdfBuffer, { name: fileName });

      } catch (error) {
        console.error('Error generating PDF for student:', student.admissionNumber, error);
        throw error;
      }
    };

    // Limit concurrency to avoid overloading the system
    const generatePdfsInBatches = async (students, limit) => {
      let index = 0;
      while (index < students.length) {
        const batch = students.slice(index, index + limit);
        await Promise.all(batch.map(generatePdfForStudent));
        index += limit;
      }
    };

    // Generate all PDFs with limited concurrency
    await generatePdfsInBatches(students, CONCURRENT_LIMIT);
    ...
    // Log when the zip stream is closed
    zipStream.on('close', () => {
      console.log('Zip stream closed.');
    });

    // Log any errors with the archiver
